...
export default function Hero({
  ...
  const hasAppleStoreLink = Boolean(
    appleStoreLink && appleStoreLink.trim().length > 0,
  );
  const hasGooglePlayLink = Boolean(
    googlePlayLink && googlePlayLink.trim().length > 0,
  );
  ...
  return (
    <section className="grid lg:grid-cols-2 gap-10 container mt-5">
      {/* Label */}
      <div className="space-y-10">
        ...
        <h1 className="text-xl sm:text-2xl lg:text-5xl font-medium flex flex-col gap-1">
        ...
        {/* Desc */}
        <p className="font-light text-black text-lg/10">{t("description")}</p>
        {/* Actions */}
        <Button
          asChild
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <Carousel
          setApi={setApi}
          className="relative flex-1 overflow-hidden rounded-4xl min-h-[320px] sm:min-h-[420px] lg:min-h-[520px] max-h-179.25"
        >
        ...
        <div className="self-start *:size-12 transition-transform *:hover:scale-110 flex sm:flex-col gap-3">
          ...
          >
            <a
              href={appleStoreLink || undefined}
              aria-label={tCommon("appleStore")}
            >
          ...
          >
            <a
              href={googlePlayLink || undefined}
              aria-label={tCommon("googlePlay")}
            >
